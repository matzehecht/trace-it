...
import { InitStorageOptions, PRECISION } from '../models';
import * as Storage from '../storage';
import { TransactionInstance } from '../storage/models';
import { TransactionResult, TransactionResultCollection } from './models';

export async function analyze(storage: Storage.Drivers, storageOptions: InitStorageOptions) {
  const StorageAdapter: Storage.Adapter = Storage.getStorage(storage, storageOptions);

  const result = await analyzeChildren(StorageAdapter, []);
  return result;
}

async function analyzeTransaction(storage: Storage.Adapter, semanticId: string[]): Promise<TransactionResult> {
...
async function analyzeChildren(storage: Storage.Adapter, semanticId: string[]): Promise<TransactionResultCollection> {
